    ...
    { //columns in multiply
      for (k = 0; k < local_size;)
      {
        if (k < local_size - 8)
        {
          ...
          sum += first[i][k + 1] * second[k + 1][j];
          sum += first[i][k + 2] * second[k + 2][j];
          sum += first[i][k + 3] * second[k + 3][j];
          sum += first[i][k + 4] * second[k + 4][j];
          sum += first[i][k + 5] * second[k + 5][j];
          sum += first[i][k + 6] * second[k + 6][j];
          sum += first[i][k + 7] * second[k + 7][j];
          k += 8;
        }
        else
        {
