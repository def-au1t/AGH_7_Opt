    ...
    for (j = i; j < size; j++)
    {
      ...
      for (k = 0; k < i;)
      {
        ...
        if (k < max(i - 8, 0))
        {
          ...
          sumU += (L[IDX(i, k + 1, size)] * U[IDX(j, k + 1, size)]);
          sumU += (L[IDX(i, k + 2, size)] * U[IDX(j, k + 2, size)]);
          sumU += (L[IDX(i, k + 3, size)] * U[IDX(j, k + 3, size)]);
          sumU += (L[IDX(i, k + 4, size)] * U[IDX(j, k + 4, size)]);
          sumU += (L[IDX(i, k + 5, size)] * U[IDX(j, k + 5, size)]);
          sumU += (L[IDX(i, k + 6, size)] * U[IDX(j, k + 6, size)]);
          sumU += (L[IDX(i, k + 7, size)] * U[IDX(j, k + 7, size)]);

          if (i != j)
          {
            sumL += (L[IDX(j, k, size)] * U[IDX(i, k, size)]);
            sumL += (L[IDX(j, k + 1, size)] * U[IDX(i, k + 1, size)]);
            sumL += (L[IDX(j, k + 2, size)] * U[IDX(i, k + 2, size)]);
            sumL += (L[IDX(j, k + 3, size)] * U[IDX(i, k + 3, size)]);
            sumL += (L[IDX(j, k + 4, size)] * U[IDX(i, k + 4, size)]);
            sumL += (L[IDX(j, k + 5, size)] * U[IDX(i, k + 5, size)]);
            sumL += (L[IDX(j, k + 6, size)] * U[IDX(i, k + 6, size)]);
            sumL += (L[IDX(j, k + 7, size)] * U[IDX(i, k + 7, size)]);
          }
